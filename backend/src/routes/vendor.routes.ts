...
import { sendVendorApprovedEmail } from "../services/email.service";
...
router.put(
  ...
  async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      // Check if approved (changed from INACTIVE to ACTIVE)
      if (existing.status === "INACTIVE" && vendor.status === "ACTIVE") {
        try {
          await sendVendorApprovedEmail(vendor.email, vendor.companyName);
        } catch (err) {
